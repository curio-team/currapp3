    ...
    <div class="leerplan" style="display: grid; grid-template-columns: auto repeat({{ $uitvoer->vakken->count() }}, 1fr) auto; column-gap: 1rem;">
        ...
        @foreach($uitvoer->vakken as $vak)
            ...
            @foreach ($vak->modules as $module)
                <div class="module p-2 text-center" style="background-color: {{ $module->parent->leerlijn->color }}; grid-column: {{ $loop->parent->iteration+1 }}; grid-row: {{ $module->pivot->week_start+1 }} / {{ $module->pivot->week_eind+2 }};">
